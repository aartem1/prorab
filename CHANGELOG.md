...
Вторая пара команд в ветке тех-качества — **статическое качество проходами-храповиком**.
Дополняет структурную пару (`audit`/`refactor`) отдельной парой под другую природу долга:
линтеры, тайпчекеры, форматтеры, мёртвый код, безопасность и **гейты** (pre-commit/CI).
Конвейер: `lint-audit → LINT → lint-fix (повторять по батчам) → /prorab:announce`.

- **Ключевая идея — храповик.** В отличие от структурного рефакторинга, статический долг
  монотонно фиксируется: включил правило, свёл нарушения к нулю, **запер гейтом** → уровень
...
- **`/prorab-tech:lint-audit` (новая команда, `commands/lint-audit.md`):** инвентаризирует
  тулинг (что есть / сломано / отсутствует, + команды сети для страховки проходов) и
  прогоняет все доступные анализаторы read-only через `Workflow`; для отсутствующих оценивает
  «стоимость включения» на щадящей планке. Кластеризует находки в батчи и упорядочивает
  лестницей по тирам: **A** нулевой-риск автофикс → **B** провод/починка инструментов на
  проходящей базе → **C** гейт на текущем уровне (высший рычаг) → **D** инкрементальный ратчет
  строгости. Скоринг `польза × безопасность × объём × уверенность` + DAG пред-условий;
  состязательно верифицирует behavior-preserving и «один проход». Кода не трогает; результат —
  `tasks/audits/LINT-<slug>.md` (инвентарь тулинга + роадмап батчей + полная спека батча #1).
- **`/prorab-tech:lint-fix` (новая команда, `commands/lint-fix.md`):** выполняет **ОДИН** батч
  под ключ через ultracode-Workflow. **Prime directive — сохранение поведения + запертый
  храповик:** снять конечный класс нарушений, доказать эквивалентность (зелёный baseline
  тестов/сборки/тайпчека + состязательный поиск дрейфа + дифференциальный прогон где уместно),
  **добавить гейт (pre-commit/CI) и sabotage-пробой доказать, что он краснеет на регрессе**.
  ...
  `lint-fix <id>` — конкретный батч, `lint-fix` без аргумента — auto-pick следующего несделанного
  с выполненными пред-условиями. Результат — код + гейт + `tasks/IMPL-lint-<slug>.md`; отмечает
  батч done в LINT-плане.

## prorab-tech 0.1.0

Новый плагин `prorab-tech` — отдельная ветка **тех-качества** со своим неймспейсом
...
- **`/prorab-tech:audit` (новая команда, `commands/audit.md`):** многоагентный аудит
  кодовой базы. Multi-modal sweep по классам смеллов (дублирование, сложность, нарушения
  слоёв/связность, мёртвый код, надёжность, перф, дыры покрытия, конвенции, типизация) +
  churn×complexity по git-истории. Кластеризация/дедуп, скоринг по
  ...
  трогает; результат — `tasks/audits/AUDIT-<slug>.md` (ранжированный бэклог + полная спека
...
- **`/prorab-tech:refactor` (новая команда, `commands/refactor.md`):** безопасная починка
  ...
  — нет рефакторинга: сначала характеризационные тесты, пиннящие текущее поведение (зелёные
  на старом коде, квирки сохраняются), затем мелкие behavior-preserving шаги под зелёным.
