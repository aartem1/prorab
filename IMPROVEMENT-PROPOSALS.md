...
**Замер 2026-07-28** (`wc -c`, оценка `байты / 3,6`): девять команд — 1 435 строк и 189 421 символ,
около 52 тысяч токенов. Оценка «4,5–5,5 тысячи на тяжёлую команду» была занижена: `build` — 9,2k,
`refactor` — 9,1k, `lint-fix` — 8,4k, `lint-audit` — 8,1k, `audit` — 6,8k, плюс общий
`references/project-knowledge.md` ≈ 3,3k. То есть тяжёлая команда занимает 10–12,5 тысяч токенов
до первого прочитанного файла. Важное уточнение к самой модели расхода: делегированный субагент
**не** получает тело команды — только свой бриф (1–2k), поэтому эти 10–12,5k относятся к главному
контексту, а не к каждому исполнителю.

**Замер после `prorab 0.13.0 · prorab-tech 0.12.0`** (тело команды + загружаемые ею контракты):
суммарно по девяти командам 74,1k → 67,9k. Общий контракт разделён на три файла, и команда грузит
только применимые: `ask` −33 %, `announce` −27 %, `refine` −21 %, `audit` и `lint-audit` −8 %,
`quick` ≈ без изменений. Тяжёлые исполнители сдвинулись мало (−1…−3 %): их основной объём — проза
Phase 0.5, специфичная для каждой команды, то есть её сокращение это переписывание, а не устранение
дублей. Это и осталось главным резервом вместе с секциями `What NOT to do`.
...
| 7 | ◐ | P1 | Заменить механических LLM-агентов детерминированными scripts | 4 | 5 | M | Контракт `Deterministic steps`: `prorab 0.12.0` / `prorab-tech 0.11.0`; отдельных скриптов в плагинах нет |
...
| 15 | ◐ | P2 | Добавить artifact-aware `/prorab:check` | 4 | 2 | M | Внешняя половина — `/prorab:verify` (`prorab 0.14.0`); артефактная сверка не реализована |
| 16 | ◐ | P0 | Добавить компактный runner для test/build output | 3 | 5 | M | Контракт `Run output discipline`: `prorab 0.12.0` / `prorab-tech 0.11.0`; исполняемой обёртки нет |
...
| 21 | ☐ | P1 | Журнал фактического расхода на контекст в артефактах (калибровка порога заполнения) | 3 | 3 | S | — |
| 22 | ☐ | P1 | Оценка заполнения контекста перед диспатчем и дробление исполнителя по шву | 4 | 3 | L | — |

### 21–22 — вторая ось бюджета (заполнение контекста)

`prorab 0.12.0` / `prorab-tech 0.11.0` ввели контракт `Context hygiene`: он ограничивает, чем
**наполняется** каждый контекст (digest вместо сырого вывода, капсула вместо дампа, разгруженный
главный цикл выше tier S). Это первая половина оси. Осталось две:

- **21 — журнал.** Одна строка на делегированный контекст в `IMPL`/`QUICK`: тип, оценка входа в
  байтах, израсходованные ходы, был ли сплит. Дешёвая часть отложенного предложения №2 — без
  eval-набора. Нужна, чтобы порог заполнения был откалиброван по факту, а не назначен.
- **22 — оценка и дробление.** Детерминированный подсчёт входа перед запуском исполнителя
  (преамбула + `wc -c` по файлам брифа + ожидаемый вывод) и три исхода: **fits** — запускать как
  есть; **split** — резать по шву (узлы DAG, подсистемы, файловые группы, где DoD-пункт остаётся
  целиком в одной группе) с передачей состояния капсулой; **decompose** — шва нет, значит задача
  недодроблена на уровне идеи, и это возврат в `refine`, а не принудительный шардинг. Резать
  связный edit, изменение контракта вместе с его call-sites, или DoD-пункт отдельно от его теста —
  запрещено: именно так дробление начинает вредить качеству вместо того, чтобы его повышать.
  Зависит от 21: без замеров непонятно, какие исполнители реально подходят к порогу.
...
**Частично закрыто `/prorab:verify` (`prorab 0.14.0`).** Реализована внешняя половина: скоуп
определяется командами (`git status --porcelain`, ветка против merge base, последний активный
IMPL/QUICK) и уточняется у пользователя только при реальной неопределённости; поведение проверяется
чёрным ящиком контекстом, который не видел реализацию; ожидаемые значения берутся из требования, а не
из вывода системы; отдельная фаза проверяет, что тесты действительно падают при регрессии проверенного
поведения, и дописывает недостающие. Осталась именно артефактная сверка: сопоставление Scope IN/OUT из
IDEA с фактическим change set, покомпонентный DoD-учёт и незакрытые `[?:…]`. Она по-прежнему ждёт
формальной схемы артефактов (предложение 6) и остаётся дешёвой read-only надстройкой над уже
существующим `verify`.
