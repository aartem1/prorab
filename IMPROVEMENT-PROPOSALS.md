...
| 7 | ◐ | P1 | Заменить механических LLM-агентов детерминированными scripts | 4 | 5 | M | Контракт `Deterministic steps`: `prorab 0.12.0` / `prorab-tech 0.11.0`; probe-скелет для веба: `prorab 0.15.0`; утилит `prorab-*` нет |
...
| 10 | ◐ | P1 | Использовать LSP и встроенные `/verify`, `/run`, `/simplify`, `/review` | 4 | 3 | S–M | Веб-проверка ушла с интерактивного браузера на headless: `prorab 0.15.0`; LSP-навигация не реализована |
...
| 21 | ◐ | P1 | Журнал фактического расхода на контекст в артефактах (калибровка порога заполнения) | 3 | 3 | S | Ledger сегментного прогона ведёт `Context ledger` (сегмент, вид контекста, ходы): `prorab 0.16.0`; на S/M/L-прогонах журнала нет |
| 22 | ◐ | P1 | Оценка заполнения контекста перед диспатчем и дробление исполнителя по шву | 4 | 3 | L | Половина «резать по шву» — тир XL и `references/segmented-run.md`: `prorab 0.16.0`; байтовой оценки перед диспатчем нет |
...
**Что из этого закрыл `prorab 0.16.0`.** Тир **XL** и `references/segmented-run.md` реализовали
исход **split** — но на уровне идеи, а не исполнителя: шов выбирается в `refine`, где его ещё можно
выбрать честно, а не навязывается уже запущенному исполнителю. Правила шва — те же три запрета
(связная правка, контракт вместе с call-sites, DoD-пункт вместе с тестом) — и тот же исход
**decompose**: шва нет — значит задача недодроблена, и это ещё один круг уточнения, а не шардинг.
`Context ledger` в ledger'е сегментного прогона даёт журнал №21, но только на XL. Не сделано:
детерминированная байтовая оценка входа перед диспатчем (исход **fits** остаётся суждением) и журнал
на обычных S/M/L-прогонах.
...
**Частично реализовано в `prorab 0.15.0` — но обратным решением, чем предполагал этот пункт.**
Предложение советовало опираться на встроенные возможности; для веб-проверки встроенный путь
(интерактивный браузер) оказался и самым медленным, и самым слабым доказательством: моделью там
управляет скриншот, поэтому каждый шаг — round-trip с картинкой, а тост «Сохранено» выдаётся за
доказательство сохранения. Контракт `references/web-probing.md` делает дефолтом headless-прогон:
модель один раз **пишет** сессию потребителя и судит структурированный результат. Интерактивная
визуальная сессия осталась уровнем L3 — только по названному триггеру, и он попадает в отчёт.
Вывод для остальных пунктов списка: встроенное средство берём, когда оно даёт доказательство не хуже,
а не потому что оно встроенное.
