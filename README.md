...
```
сырая идея ──/prorab:refine──▶ tasks/ideas/IDEA-<slug>.md ──/prorab:build──▶ реализация под ключ
```
...
Команды `refine` и `build` можно использовать и по отдельности, но задуманы как пара:
сначала прорабатываем «чертёж», потом строим.
...
После установки команды доступны глобально во всех проектах как `/prorab:refine` и
`/prorab:build`.
