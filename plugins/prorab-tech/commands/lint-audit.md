...
**Опора и посыл (ultracode):** свободно используй `Workflow` для fan-out (параллельные прогоны анализаторов по линзам) и состязательной верификации того, что каждый batch действительно безопасен и behavior-preserving. Токены — не ограничение; безопасность прохода и корректность плана — ограничение.
