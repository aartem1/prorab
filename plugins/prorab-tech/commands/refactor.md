...
**Опора и посыл (ultracode):** свободно используй `Workflow` для fan-out (разведка, поиск дрейфа, верификация) и состязательной проверки. Цель — доказанная эквивалентность + доказанное улучшение, а не самый быстрый/дешёвый путь. Токены — не ограничение; сохранность поведения — ограничение.
...
4. **Кратко изложи план-наброску** (сеть → шаги → верификация эквивалентности) — для прозрачности, **не как checkpoint**. Сразу переходи к Фазе 1.
