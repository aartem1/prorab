...
4. **The gate state is proven.** For C and post-C A/B/D, the allocated independent verifier injects a plausible violation of the newly enforced class, runs the relevant existing gate entrypoint, confirms it **goes red**, and removes the mutation without including it in a commit. The gate didn't go red → the ratchet isn't locked at the new bar: **fix the gate**, the pass isn't done. For pre-C A/B, verify instead that no locked claim or unrelated gate edit was introduced; sabotage is not required because no gate coverage changed.
