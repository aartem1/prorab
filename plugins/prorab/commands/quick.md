...
**Your context is the executor's context.** Because there is no orchestrator here, the main-loop rule of the `Context hygiene` contract does not apply — reading, editing and running checks yourself is the intended shape of this lane. The other two limits do apply, and one of them matters more here than anywhere: `Run output discipline`. Your two contexts are all you get, and a single failing suite can spend one of them, so send every run's output to a file outside the working tree and keep only the digest — command, exit code, counters, one identifying line per failure. The verifier's return is a capsule of claims and pointers, not a re-listing of the diff you already have.
