...
1. **If `$ARGUMENTS` names the scope, take it:** a slug or path to `tasks/IMPL-*.md` / `tasks/quick/QUICK-*.md` / `tasks/revisions/REVISION-*.md` / `tasks/ideas/IDEA-*.md`; a branch or base ref; a commit range; `uncommitted`; or a free description of the functionality. Do not select `tasks/archive/**` by default — only when the user names an archived task explicitly. A `REVISION-*` names the requirements **as they now stand**: its `History` records the remarks that moved them after the original build and its `Invariants` say which are pinned, so where it and the IMPL disagree the revision wins and the IMPL is the historical statement.
2. **Otherwise derive the candidates with commands, not impressions** (`Deterministic steps`): `git status --porcelain` for the uncommitted set; the current branch and its base (`git rev-parse --abbrev-ref HEAD`, the default branch from `git symbolic-ref refs/remotes/origin/HEAD` or `main`/`master`, then `git diff --name-only <base>...HEAD` and `git log --oneline <base>..HEAD`); the most recent active IMPL/QUICK/REVISION artifact. `git diff --stat` gives the change class.
