...
- **Скептицизм служит скоупу и DoD, а не себе.** Не переспрашивай про то, что не влияет на скоуп/DoD. Если два раунда подряд ключевые 🟥/🟧 не двигаются или пользователь уклоняется — предложи свести, зафиксировав оставшееся как открытые пункты / `[?:…]`, а не гоняй новый круг.
- **Опирайся на код, а не на фантазию.** Прежде чем спорить про реализацию — читай файлы, проверяй модели, эндпоинты, миграции. Если не нашёл — так и говори: «в коде этого не вижу, уточни».
...
- Найди в репозитории релевантные файлы (модели, схемы, сервисы, эндпоинты, миграции, фронтовые компоненты, типы). Используй `Grep`/`Glob`, при объёме > 3 запросов — делегируй `Agent` (`subagent_type: Explore`).
- Зафиксируй для себя: **что уже есть** (готовое к переиспользованию), **что придётся менять**, **что конфликтует** с идеей.
- Если идея ломает существующий контракт (API, схему БД, поведение пайплайна) — это отдельный пункт для обсуждения, не закапывай.

Кратко отчитайся пользователю одним сообщением: «Посмотрел X, Y, Z — вот что нашёл релевантного: …».

### Фаза 3 — Итеративный рефайнмент
...
3. Получив ответ — **обнови карту неясностей** (что закрыто, что появилось нового), если изменилось понимание скоупа — скажи об этом явно.
...
- предлагать out-of-scope-ампутации («это можно вынести в отдельную итерацию, согласен?»).
...
### Риск-спайки (проверить ДО стройки: риск → как проверить)
- …
...
- Ответы пользователя перестали менять скоуп — только уточняют детали реализации.
