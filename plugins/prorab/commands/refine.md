...
**Model and effort — the one command that is deliberately not pinned.** Every other Prorab entrypoint sets `model`/`effort` in its frontmatter so the run stops inheriting whatever session the user was in. `refine` does not, and the reason is its shape, not an oversight. A frontmatter override lasts **only for the current turn**: the session model returns on the user's next message. A command that advances in many rounds would therefore hop between the pinned model and the session model on every round — and because a prompt cache is scoped to one model, each hop rewrites the whole conversation prefix cold. On a dialogue this long that costs more than the cheaper model saves. So `refine` runs on the **session model**, by design: this is also the phase where the product decisions are actually made, and it is the right place to spend a strong one. What it does cheapen is delegated recon — a built-in `Explore` context inherits the main conversation's model, so **pass `model` explicitly on every recon call** rather than letting an Opus session make lookup work Opus. If this ever becomes a single-pass command, pinning it becomes correct; reshaping it is not part of that decision.
